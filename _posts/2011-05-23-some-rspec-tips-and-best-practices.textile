...
Note the use of the @@user@ instance variable for the user.   That certainly works, but has the problem of introducing subtle bugs since @@instance@ variables spring into existence as @nil@ when first referenced, which is probably not what you wanted.
...
* Will raise an exception if the variable a @let@ is supposed to define is not set.
...
There is also a @let!{}@ version that tells RSpec to evaluate your block before your examples run - basically turning it in to a before hook. Pretty sweet!
...
There are lots of "other @--tag@ command line options":http://relishapp.com/rspec/rspec-core/v/2-6/dir/command-line/tag-option to let you include or exclude specs based on your rags.